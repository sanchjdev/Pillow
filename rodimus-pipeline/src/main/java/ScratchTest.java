import java.util.List;


import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;

import com.pillowsuite.service.RabbitMqConsumer;
import com.pillowsuite.shared.messaging.enums.RabbitMQueue;
import com.pillowsuite.shared.model.FullMover;


public class ScratchTest {
    public static void main(String[] args) throws Exception {
        RabbitMQueue queue = RabbitMQueue.TOP_MOVERS;
        RabbitMqConsumer consumer = new RabbitMqConsumer();
        consumer.consume(queue.getName());

        ObjectMapper mapper = new ObjectMapper();
        List<FullMover> fullMovers = mapper.readValue(consumer.message, new TypeReference<List<FullMover>>() {});

        System.out.println(fullMovers.get(1).getDirection());



    }
}